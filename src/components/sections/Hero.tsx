...
export function Hero() {
  ...
  return (
    <section
      ref={heroRef}
      className="relative min-h-[calc(100vh-64px)] flex items-center justify-center overflow-hidden mesh-bg"
    >
      {/* Cursor glow overlay */}
      <div
        ref={glowRef}
        ...
        <div className="flex items-center justify-center gap-3 mt-8 text-sm text-gray-600 animate-fade-up" style={{ animationDelay: "0.6s" }}>
          <div className="flex -space-x-2">
            {["#3B82F6", "#8B5CF6", "#10B981", "#F59E0B", "#EF4444"].map((color, i) => (
              <div key={i} className="h-7 w-7 rounded-full border-2 border-surface" style={{ backgroundColor: color }} />
            ))}
          </div>
          <span>Join <strong className="text-gray-400">50,000+</strong> creators already learning</span>
        </div>
      </div>
    </section>
  );
