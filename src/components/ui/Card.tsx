...
function Card({ className, hover, padding = "md", children, ...props }: CardProps) {
  const p = { none: "", sm: "p-4", md: "p-6", lg: "p-8" }[padding];
  return (
    <div className={cn("rounded-2xl bg-surface-1 border border-white/[0.06]", p, hover && "card-hover cursor-pointer", className)} {...props}>
      {children}
    </div>
  );
}
