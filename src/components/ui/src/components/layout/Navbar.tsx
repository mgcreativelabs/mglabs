...
export function Navbar() {
  ...
  return (
    <header className={cn("fixed top-0 left-0 right-0 z-50 transition-all duration-300", scrolled ? "bg-surface/90 backdrop-blur-xl border-b border-white/[0.06] shadow-2xl shadow-black/50" : "bg-transparent")}>
            ...
            {navLinks.map((link) => (
              <Link key={link.href} href={link.href}
                className={cn("flex items-center gap-1.5 px-3 py-2 rounded-lg text-sm font-medium transition-all",
                  pathname === link.href ? "text-white bg-surface-3" : "text-gray-400 hover:text-white hover:bg-surface-2")}>
          ...
          <div className="hidden lg:flex items-center gap-3">
            {!loading && (user ? (
              ...
              <>
                ...
                <Link href="/signup" className="px-4 py-2 rounded-xl bg-gradient-brand text-white text-sm font-semibold shadow-lg shadow-brand-blue/20 hover:shadow-brand-blue/40 hover:scale-[1.02] transition-all">Get started free</Link>
              </>
            ))}
          </div>
