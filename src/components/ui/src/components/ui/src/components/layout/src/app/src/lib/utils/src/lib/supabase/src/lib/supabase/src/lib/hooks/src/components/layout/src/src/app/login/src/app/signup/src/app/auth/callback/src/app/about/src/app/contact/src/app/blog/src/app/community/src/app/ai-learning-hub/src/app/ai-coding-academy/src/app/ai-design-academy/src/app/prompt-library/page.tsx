...
const PROMPTS = [
  { id: "1", title: "Expert Blog Post Writer", description: "Write SEO-optimized, engaging blog posts that rank and convert.", content: "You are an expert content strategist with 10 years of experience. Write a comprehensive, SEO-optimized blog post about [TOPIC]. The post should be [LENGTH] words, target the keyword [KEYWORD], include [NUMBER] subheadings, use a [TONE] tone, and end with a clear call-to-action.", category: "Writing", difficulty: "Beginner", likes: 2847, model: "GPT-4", featured: true },
  { id: "2", title: "Senior Code Reviewer", description: "Get expert-level code review with security and performance feedback.", content: "Act as a senior software engineer. Review the following code and provide: 1) Security vulnerabilities, 2) Performance bottlenecks, 3) Code quality issues, 4) Missing edge cases, 5) A refactored version of critical sections. Be specific and explain your reasoning.\n\n[PASTE CODE HERE]", category: "Coding", difficulty: "Advanced", likes: 1923, model: "Claude", featured: true },
  { id: "3", title: "Brand Identity Creator", description: "Generate a complete brand identity including voice, values and visual direction.", content: "You are a world-class brand strategist. Create a complete brand identity for [COMPANY NAME] — a [BUSINESS TYPE] targeting [AUDIENCE]. Include: 1) Brand tagline options, 2) Brand personality, 3) Voice guidelines with examples, 4) Core values, 5) Visual direction, 6) Brand story.", category: "Business", difficulty: "Intermediate", likes: 1542, model: "GPT-4", featured: false },
  { id: "4", title: "Midjourney Prompt Master", description: "Generate stunning, detailed Midjourney prompts for any visual style.", content: "Generate a detailed Midjourney prompt for: [DESCRIPTION]. Include: subject details, artistic style references, lighting description, color palette, camera perspective, quality modifiers. Format: [subject], [style], [lighting], [colors], [perspective], --ar 16:9 --v 6 --q 2", category: "Design", difficulty: "Beginner", likes: 3210, model: "Midjourney", featured: true },
  { id: "5", title: "Email Campaign Strategist", description: "Write high-converting email sequences that open, engage and convert.", content: "You are a conversion copywriter with proven 40%+ open rates. Write a [NUMBER]-email sequence for [PRODUCT] targeting [AUDIENCE]. Each email needs: subject line + preview text, personalized opening, value-driven body, clear CTA, P.S. line. Sequence: welcome, education, social proof, objection handling, offer.", category: "Marketing", difficulty: "Intermediate", likes: 1876, model: "Claude", featured: false },
  { id: "6", title: "Python Automation Builder", description: "Build production-ready Python scripts to automate repetitive tasks.", content: "You are a Python automation expert. Create a production-ready Python script that [TASK]. Requirements: Python 3.10+ syntax, error handling and logging, type hints throughout, configurable via environment variables, requirements.txt, docstrings for all functions, README with usage instructions.", category: "Coding", difficulty: "Intermediate", likes: 1654, model: "GPT-4", featured: false },
];
    ...
    <div className="flex flex-col rounded-2xl bg-surface-1 border border-white/[0.06] hover:border-brand-blue/30 transition-all overflow-hidden">
      <div className="p-5 flex-1">
        <div className="flex items-center gap-2 flex-wrap mb-3">
          ...
          <span className={`text-xs px-2 py-1 rounded-full border font-medium ${prompt.difficulty === "Beginner" ? "bg-green-500/10 text-green-400 border-green-500/20" : prompt.difficulty === "Advanced" ? "bg-red-500/10 text-red-400 border-red-500/20" : "bg-yellow-500/10 text-yellow-400 border-yellow-500/20"}`}>{prompt.difficulty}</span>
          {prompt.featured && <span className="text-xs px-2 py-1 rounded-full bg-gradient-brand text-white font-medium flex items-center gap-1"><Star className="h-2.5 w-2.5" />Featured</span>}
        ...
        <p className="text-gray-500 text-sm mb-4">{prompt.description}</p>
      ...
      <div className="border-t border-white/[0.04] p-4 flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <button className="p-2 rounded-lg text-gray-500 hover:text-white hover:bg-surface-3 transition-colors"><Bookmark className="h-4 w-4" /></button>
          <button onClick={handleCopy} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${copied ? "bg-green-500/10 text-green-400 border border-green-500/20" : "bg-gradient-brand text-white hover:scale-[1.02]"}`}>
