...
// ─────────────────────────────────────────────
// Next.js 16 requires any component that calls
// useSearchParams() to be wrapped in <Suspense>.
// We isolate the param-reading into a sub-component
// so the login page itself can remain statically
// rendered (the outer shell), while only the inner
// form (which actually needs the ?next= param) is
// suspended during build pre-rendering.
// ─────────────────────────────────────────────
...
function LoginForm() {
  ...
  // Only allow same-site relative redirects — never redirect to an
  // external URL based on user-controlled query params.
