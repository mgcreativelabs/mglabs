    ...
    <div className="pt-16 min-h-screen">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-10">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-10">
          ...
          <button onClick={() => signOut()} className="px-4 py-2 rounded-xl bg-surface-2 border border-white/10 text-gray-400 text-sm font-medium hover:text-white transition-colors w-fit">
            Sign out
          </button>
        </div>
        ...
        <div className="grid lg:grid-cols-3 gap-6">
                ...
                {quickLinks.map(({ icon: Icon, label, href, color }) => (
                  <Link key={href} href={href}>
                    <div className="flex items-center gap-3 p-3.5 rounded-xl bg-surface-2 border border-white/[0.04] hover:border-brand-blue/30 hover:bg-surface-3 transition-all group">
                      ...
                      <ArrowRight className="h-3.5 w-3.5 text-gray-600 group-hover:text-brand-blue ml-auto" />
                    </div>
                  </Link>
                ))}
              </div>
          ...
          <div className="space-y-5">
            ...
            <div className="rounded-2xl bg-surface-1 border border-brand-purple/20 p-5">
              ...
              <button className="w-full py-2.5 rounded-xl bg-gradient-brand text-white text-sm font-semibold hover:scale-[1.02] transition-all">Upgrade — $9/mo</button>
            </div>
          </div>
        </div>
      </div>
